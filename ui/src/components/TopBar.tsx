...
import { AuthStatus } from '../api.ts'; // Import AuthStatus type
...
const TopBar: React.FC<TopBarProps> = ({ authStatus, handleLogin }) => {
  console.log('[TopBar] Received authStatus:', authStatus); // <-- Add log
        ...
        {isLoggedIn ? (
          <>
            ...
            <div className="text-sm">
              <div>{authStatus.profile.username}</div>
          ...
          <button
            onClick={handleLogin}
            className="px-4 py-1.5 bg-zinc-700 hover:bg-zinc-600 text-gray-200 rounded text-sm font-medium border border-zinc-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-zinc-800 focus:ring-zinc-500" /* Updated classes */
          >
            Login
          </button>
        )}
